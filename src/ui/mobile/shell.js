...
import { addTrack, addClip, removeClip, splitClip, setKeyframe, createKeyframe, totalDuration } from '../../engine/edl.js';
...
class MobileShell {
  ...
  _bindPanelSwipe() {
    ...
    if (!track) return;
    let startX, startY, dir = null, dragging = false, skipSwipe = false;

    track.addEventListener('touchstart', (e) => {
      // Don't intercept touches that start on interactive controls — let them work natively.
      skipSwipe = !!e.target.closest('input, select, textarea, button, label, a');
      if (skipSwipe) return;
    ...
    track.addEventListener('touchend', (e) => {
      if (skipSwipe || dir !== 'h' || !dragging) return;
  ...
  async _importFile() {
    ...
    input.type = 'file';
    input.accept = 'video/*,audio/*,image/*,.mp4,.mov,.avi,.mkv,.webm,.mp3,.wav,.aac,.m4a,.ogg,.flac,.jpg,.jpeg,.png,.gif,.webp';
    input.multiple = true;
    input.addEventListener('change', async () => {
      const files = Array.from(input.files ?? []);
      let failed = 0;
      for (const file of files) {
        try { await this._ingestFile(file); }
        catch (err) { failed++; console.error('Import failed:', file.name, err); }
      }
      this._renderPanel(0);
      if (failed > 0) {
        this._showInfoDialog('Import Failed',
          `${failed} file${failed > 1 ? 's' : ''} could not be imported. Device storage may be full or restricted. Try a smaller file or reload the page.`);
      }
    });
    input.click();
  }
  ...
  async _ingestFile(file) {
    ...
    const asset = {
      ...
      name: file.name, type, mimeType: file.type || 'application/octet-stream',
  ...
  _addAssetToTimeline(assetId) {
    ...
    if (!track) {
      ...
      track = this._pm.project.tracks.find((t) => t.type === trackType && !t.locked);
    }
    if (!track) return;
    const trackId   = track.id;
    const clipDur   = asset.duration || (asset.type === 'image' ? 5 : 5);
    const cmd = this._history.snapshotCommand('Add clip', (proj) => {
      const startTime = totalDuration(proj);
      addClip(proj, trackId, { assetId: asset.id, startTime, duration: clipDur });
    });
  ...
  _renderClipPanel(panel) {
    ...
    const atPlayhead = this._currentTime > clip.startTime && this._currentTime < clip.startTime + clip.duration;
    panel.innerHTML = `
      <div class="pm-m-panel-inner">
        <div class="pm-m-panel-header">
          <span>${escHtml((clip.name ?? 'Clip').slice(0, 22))}</span>
          <div style="display:flex;gap:6px">
            <button class="pm-m-pill-btn" id="pm-m-split-clip" ${atPlayhead ? '' : 'disabled'} title="Split clip at playhead">✂ Split</button>
        ...
        <div class="pm-m-fade-row">
          <button class="pm-m-pill-btn pm-m-fade-btn" id="pm-m-fade-in">↗ Fade In</button>
          <button class="pm-m-pill-btn pm-m-fade-btn" id="pm-m-fade-out">↘ Fade Out</button>
          <button class="pm-m-pill-btn pm-m-fade-btn" id="pm-m-fade-clear">✕ Clear</button>
        </div>
    ...
    panel.querySelector('#pm-m-split-clip')?.addEventListener('click', () => {
      const project = this._pm.project;
      if (!project) return;
      const t = this._currentTime;
      const cmd = this._history.snapshotCommand('Split clip', (proj) => splitClip(proj, clip.id, t));
      this._history.execute(cmd);
      this._selectedClip = null;
      this._renderTimeline();
    ...
    panel.querySelector('#pm-m-fade-in')?.addEventListener('click', () => {
      const c = this._findClip(clip.id);
      if (!c) return;
      const fadeDur = Math.min(0.5, c.duration * 0.4);
      this._history.execute(this._history.snapshotCommand('Fade In', (proj) => {
        const liveClip = proj.tracks.flatMap((t) => t.clips).find((x) => x.id === c.id);
        if (!liveClip) return;
        setKeyframe(liveClip, 'opacity', createKeyframe({ time: liveClip.startTime, value: 0, easing: 'linear' }));
        setKeyframe(liveClip, 'opacity', createKeyframe({ time: liveClip.startTime + fadeDur, value: 1, easing: 'linear' }));
      }));
      this._pm.markDirty();
    });

    panel.querySelector('#pm-m-fade-out')?.addEventListener('click', () => {
      const c = this._findClip(clip.id);
      if (!c) return;
      const fadeDur = Math.min(0.5, c.duration * 0.4);
      this._history.execute(this._history.snapshotCommand('Fade Out', (proj) => {
        const liveClip = proj.tracks.flatMap((t) => t.clips).find((x) => x.id === c.id);
        if (!liveClip) return;
        const fadeStart = liveClip.startTime + liveClip.duration - fadeDur;
        setKeyframe(liveClip, 'opacity', createKeyframe({ time: fadeStart, value: 1, easing: 'linear' }));
        setKeyframe(liveClip, 'opacity', createKeyframe({ time: liveClip.startTime + liveClip.duration, value: 0, easing: 'linear' }));
      }));
      this._pm.markDirty();
    });

    panel.querySelector('#pm-m-fade-clear')?.addEventListener('click', () => {
      const c = this._findClip(clip.id);
      if (!c) return;
      this._history.execute(this._history.snapshotCommand('Clear opacity keyframes', (proj) => {
        const liveClip = proj.tracks.flatMap((t) => t.clips).find((x) => x.id === c.id);
        if (liveClip) delete liveClip.keyframes.opacity;
      }));
      this._pm.markDirty();
    });
  ...
  _renderExportPanel(panel) {
    ...
    startBtn.addEventListener('click', async () => {
      // Check WebCodecs availability before attempting export
      if (typeof VideoEncoder === 'undefined' || typeof OffscreenCanvas === 'undefined') {
        errDiv.textContent = 'Export requires Chrome 94+, Edge 94+, or Safari 16+. WebCodecs is not available in this browser. Try opening PeachMint in Chrome on desktop or a newer mobile browser.';
      ...
      try {
        ...
        const isLibErr = msg.includes('import') || msg.includes('fetch') || msg.includes('network') || msg.includes('muxer');
        if (msg !== 'Export cancelled') {
          errDiv.textContent = isLibErr
            ? 'Could not load the export library. Check your internet connection and try again.'
            : `Export failed: ${msg}`;
          errDiv.style.display = 'block';
        }
...
function injectStyles() {
  ...
  s.textContent = `
    ...
    .pm-m-prop-slider { flex:1; accent-color:var(--accent-peach); touch-action:none; min-width:0;
      height:28px; cursor:pointer; }
    .pm-m-fade-row { display:flex; gap:6px; flex-wrap:wrap; padding:4px 0; }
